...
export function generateReceiptEscPos(data: ReceiptData): Buffer {
  ...
  // 3. [NAME]
  // "ANNA CHEN" - Double height, bold, centered
  ...
  // "[ACT OF SERVICE]" - Normal size, bold, centered in brackets
  ...
  // 5. [QUEUE NUMBER]
  // "042" - Double width + double height, bold, centered
  ...
  // 6. [TIME]
  // "[13.39]" - Normal size, regular, centered in brackets [HH.mm]
  chunks.push(Buffer.from([GS, 0x21, 0x00])); // GS ! 0x00: Normal size
...
export async function printToBlueprintQ58D(buffer: Buffer): Promise<{ success: boolean; message?: string }> {
  ...
  const exePath = path.resolve(process.cwd(), '..', 'FE', 'scripts', 'RawPrinter.exe');
